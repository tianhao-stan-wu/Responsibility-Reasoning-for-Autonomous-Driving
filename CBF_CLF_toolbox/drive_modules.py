...
class turn:
    ...
    def check_update_module(self, x):
        ...
        if distance(x, self.waypoints_list[self.index+1]) <= self.d * 1.5 and self.index != self.len-2:
            self.index += 1
            self.set_module()

        elif distance(x, self.waypoints_list[self.index+1]) <= self.d * 1.5 and self.index == self.len-2:
...
class change_lane:
    ...
    def check_update_module(self, x):
        ...
        if distance(x, self.waypoints_list[self.index+1]) <= self.d * 2 and self.index != self.len-2:
            self.index += 1
            self.set_module()

        elif distance(x, self.waypoints_list[self.index+1]) <= self.d * 2 and self.index == self.len-2:
            print('turn completed')
            self.complete_turn = True
