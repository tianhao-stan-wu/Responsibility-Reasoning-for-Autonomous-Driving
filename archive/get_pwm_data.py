...
def game_loop(args):
    ...
    try:
        ...
        count = 0
        throttle_test_cycles = 10  # Number of throttle test repetitions
        brake_test_cycles = 5  # Number of brake test repetitions
        ...
        while True:
            ...
            if count >= 100 and count < 300 :
                # 10 cycles of random throttle application for 20 counts each

                if (count - 100) % 20 == 0:
                    control.throttle = random.choice([i * 0.1 for i in range(11)])
                    control.brake = 0  # Ensure no braking during throttle test

            elif count >= 300 and count < 500:
                # 5 cycles of random brake application for 5 counts each
             
                if (count - 300) % 20 == 0:
                    control.throttle = 0  # Ensure no throttle during brake test
                    control.brake = random.choice([i * 0.1 for i in range(11)])
