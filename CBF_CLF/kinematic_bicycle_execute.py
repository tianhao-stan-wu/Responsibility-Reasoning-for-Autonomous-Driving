...
vehicle = kb.KinematicBicycle(dynamics_params)
ds = define_system.ControlAffineSystem(vehicle)

QPoption = ccq.CbfClfQpOptions()
QPoption.set_option('u_max', np.array([10, to_radian(70)]))
QPoption.set_option('u_min', np.array([-10, -1 * to_radian(70)]))
...
def show_three_plots(xt, ut, dt, T, show=1, save=0):
    t = np.arange(0, T, dt)
    ...
    obs = plt.Circle((dynamics_params['xo'], dynamics_params['yo']), dynamics_params['ro'], color='red', fill=False, linewidth=2, linestyle='dashed')
